...
const fetchUsers = async () => {
  const res = await fetch('https://juanjobsph.onrender.com/api/users/users')
...
const handleEditSubmit = async (formData) => {
  const res = await fetch(`https://juanjobsph.onrender.com/api/users/users/${formData._id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(formData)
  })
...
const deleteUser = async (id) => {
  ...
  await fetch(`https://juanjobsph.onrender.com/api/users/users/${id}`, { method: 'DELETE' })
  await fetchUsers()
}
